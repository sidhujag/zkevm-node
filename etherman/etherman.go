...
func (etherMan *Client) sequenceBatches(opts bind.TransactOpts, sequences []ethmanTypes.Sequence, maxSequenceTimestamp uint64, lastSequencedBatchNumber uint64, l2Coinbase common.Address) (*types.Transaction, error) {
	...
	for _, seq := range sequences {
		...
		batch := etrogpolygonzkevm.PolygonRollupBaseEtrogBatchData{
			...
			Transactions:       crypto.Keccak256(seq.BatchL2Data),
...
func (etherMan *Client) sequencedBatchesEvent(ctx context.Context, vLog types.Log, blocks *[]Block, blocksOrder *map[common.Hash][]Order) error {
	...
	if sb.NumBatch != 1 {
		...
		if bytes.Equal(methodId, methodIDSequenceBatchesEtrog) {
			sequences, err = decodeSequencesEtrog(tx.Data(), sb.NumBatch, msg.From, vLog.TxHash, msg.Nonce, sb.L1InfoRoot, &SyscoinClient)
			if err != nil {
				return fmt.Errorf("error decoding the sequences (etrog): %v", err)
			}
		} else if bytes.Equal(methodId, methodIDSequenceBatchesElderberry) {
			sequences, err = decodeSequencesElderberry(tx.Data(), sb.NumBatch, msg.From, vLog.TxHash, msg.Nonce, sb.L1InfoRoot, &SyscoinClient)
			if err != nil {
...
func (etherMan *Client) sequencedBatchesPreEtrogEvent(ctx context.Context, vLog types.Log, blocks *[]Block, blocksOrder *map[common.Hash][]Order, fetcher* SyscoinClient) error {
	...
	sequences, err := decodeSequencesPreEtrog(tx.Data(), sb.NumBatch, msg.From, vLog.TxHash, msg.Nonce, fetcher)
	if err != nil {
...
func decodeSequencesElderberry(txData []byte, lastBatchNumber uint64, sequencer common.Address, txHash common.Hash, nonce uint64, l1InfoRoot common.Hash, fetcher* SyscoinClient) ([]SequencedBatch, error) {
	// Extract coded txs.
	// Load contract ABI
...
func decodeSequencesPreEtrog(txData []byte, lastBatchNumber uint64, sequencer common.Address, txHash common.Hash, nonce uint64, fetcher* SyscoinClient) ([]SequencedBatch, error) {
	// Extract coded txs.
	// Load contract ABI
